...
* По большей части, обновление направлено на улучшение производительности и исправление ошибок. В виду того, что некоторые мелкие моменты были упущены, в старой версии наблюдались баги с производительностью при длительной работе библиотеки, вызванные созданием огромного количества потоков и общим отсутствием потокобезопасности библиотеки. В этом обновлении библиотека полностью отлажена, потокобезопасна благодаря использованию `java.util.concurrent` пакета, а скорость работы увеличина в разы.
* Улучшено и упрощено взаимодействие с VK API: все запросы, делаемые с помощью библиотеки, напрямую или косвенно, как и ранее, становятся в очередь и выполняются с помощью метода `execute`, но и напрямую использовать этот метод и отдавать несколько запросов для одновременного их выполнения, синхронно или асинхронно:
```java
// Асинхронно
JSONObject params = new JSONObject();
params.put("user_ids", new JSONArray("[1,2,3]"));
params.put("fields", "photo_max_orig");
        
CallAsync call = new CallAsync("users.get", params, response -> {
    System.out.println(response);
});

group.api().execute(call);
