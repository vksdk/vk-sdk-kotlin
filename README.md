...
**Доступны следующие типы сообщений** (_в порядке приоритета вызова_): `голосовое`, `стикер-сообщение`, `с гифкой`, `с аудио`, `с видео`, `с документом` (но для гифки свой коллбэк), `сообщение с прикрепленной записью со стены`, `сообщение с прикрепленной ссылкой`, `простое текстовое сообщение` (без прикреплений).
